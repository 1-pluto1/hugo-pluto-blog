...
本研究采用公开数据源构建数据集，确保不涉及隐私问题或违规行为。所有数据均未收集个人身份信息，且严格遵循法律与伦理标准获取。在数据标注阶段，我们聘请了三位具有自然语言处理研究经验的研究生参与标注工作，按每小时约13美元的标准支付报酬（显著高于当地平均工资），并在标注人员对流程存在疑虑时开展建设性讨论。# s1: Simple test-time scaling